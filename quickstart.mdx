...
```bash cURL
curl -X POST https://app.voicehire.io/api/v1/campaigns/CAMP-20250701-ABC123/candidates \
  -H "X-API-Key: vh_live_VOTRE_CLE_API" \
  -H "Content-Type: application/json" \
  -d '{
    "candidates": [
...
```bash cURL
curl -X GET https://app.voicehire.io/api/v1/campaigns/CAMP-20250701-ABC123/status \
  -H "X-API-Key: vh_live_VOTRE_CLE_API"
```
...
response = httpx.post(f"{BASE_URL}/campaigns", headers=headers, json=campaign_data)
...
print(f"✅ {response.json()['added']} candidat(s) ajouté(s)")
